/*
 * Smart Parking System — ENTRY GATE Firmware v3.0
...
#define BACKEND_URL    "http://192.168.1.16:8000/api/esp32/entry-event"
...
void loop() {
  ...
  if (detected && !vehiclePresent) {
    ...
    Serial.println("──────────────────────────────────");
    Serial.println("[IR] TRIGGERED — vehicle detected");
    ...
    if (sent) {
      ...
      if (status == "ALLOW") {
        ...
        Serial.println("[GATE] DENIED by backend");
      }
    } else {
...
bool postImage(const uint8_t* imgBuf, size_t imgLen, String& outStatus) {
  const String boundary = "SmartParkEntry";
