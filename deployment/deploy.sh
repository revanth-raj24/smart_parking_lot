#!/bin/bash
# SmartPark VPS Deployment Script — PM2 + Nginx + MySQL
...
apt update -qq
apt install -y curl git nginx mysql-server python3 python3-pip python3-venv \
...
CREATE USER IF NOT EXISTS 'smart_user'@'localhost' IDENTIFIED BY 'CHANGE_ME_DB_PASS';
GRANT ALL PRIVILEGES ON smart_parking.* TO 'smart_user'@'localhost';
FLUSH PRIVILEGES;
SQL
echo "  ⚠  Update the MySQL password in this script and in backend/.env before going live!"
...
# ── 6. Frontend (User App) ────────────────────────────────────────────────────
echo "[6/8] Building user frontend..."
cd "$APP_DIR/frontend"
...
# ── 7. Admin App ──────────────────────────────────────────────────────────────
echo "[7/8] Building admin frontend..."
